...
- Statusikonet vises som en `ix-icon`-badge (`data-badge`): en sirkel i status-`fill` med en lys glyf oppå. Sirkelfargen følger `data-status`, og glyfen settes med `materialdesignname` (`info_i`, `check`, `priority_high`).
...
- React-laget setter `role="alert"` (danger/warning) og `aria-live="polite"` (info/success) automatisk.
