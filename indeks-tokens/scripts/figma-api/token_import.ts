...
import { colorApproximatelyEqual, parseColor } from './color';
...
function variableValueFromToken(
    ...
    localVariablesByCollectionAndName: {
        [variableCollectionId: string]: {
            [variableName: string]: LocalVariable;
        };
    }
): VariableValue {
...
function isCodeSyntaxEqual(a: VariableCodeSyntax, b: VariableCodeSyntax) {
    return (
        ...
        Object.keys(a).every((key) => a[key as keyof VariableCodeSyntax] === b[key as keyof VariableCodeSyntax])
    );
}
...
function tokenAndVariableDifferences(token: Token, variable: LocalVariable | null) {
    const differences: Partial<
        Omit<VariableCreate | VariableUpdate, 'id' | 'name' | 'variableCollectionId' | 'resolvedType' | 'action'>
    > = {};

    if (token.$description !== undefined && (!variable || token.$description !== variable.description)) {
...
export function generatePostVariablesPayload(
    ...
    Object.values(localVariables.meta.variableCollections)
    ...
    Object.entries(tokensByFile).forEach(([fileName, tokens]) => {
        ...
        Object.entries(tokens).forEach(([tokenName, token]) => {
            ...
            if (existingVariableValue === null || !compareVariableValues(existingVariableValue, newVariableValue)) {
