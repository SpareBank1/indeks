...
export function tokenFilesFromLocalVariables(localVariablesResponse: GetLocalVariablesResponse) {
    ...
    Object.values(localVariables).forEach((variable) => {
        ...
        if ('deletedButReferenced' in variable && variable.deletedButReferenced) {
            return;
        }

        const collection = localVariableCollections[variable.variableCollectionId];

        //Hent kun fargene, ikke de andre variablene i Figma
        if (collection.name !== '01 Themes' && collection.name !== '02 Semantic colors') {
            return;
        }
