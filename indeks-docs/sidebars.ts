import type { SidebarsConfig } from '@docusaurus/plugin-content-docs';

// This runs in Node.js - Don't use client-side code here (browser APIs, JSX...)

/**
 * Creating a sidebar enables you to:
 - create an ordered group of docs
 - render a sidebar for each doc of that group
 - provide next/previous navigation

 The sidebars can be generated from the filesystem, or explicitly defined here.

 Create as many sidebars as you want.
 */
const sidebars: SidebarsConfig = {
    // Design system sidebar
    designSidebar: [
        'hjem',
        {
            type: 'category',
            label: 'Kom i gang',
            collapsed: false,
            items: [
                'kom-i-gang/designer',
                'kom-i-gang/utvikler',
                'kom-i-gang/migrering',
                'kom-i-gang/kontakt',
                'kom-i-gang/bidra',
            ],
        },
        {
            type: 'category',
            label: 'Design-tokens',
            collapsed: true,
            items: [
                'grunnleggende/tokens/introduksjon',
                {
                    type: 'category',
                    label: 'Farger',
                    items: [
                        'grunnleggende/tokens/fargeskalaer',
                        'grunnleggende/tokens/farger-web',
                        'grunnleggende/tokens/farger-native',
                    ],
                },
                'grunnleggende/tokens/spacing',
                'grunnleggende/tokens/border',
                'grunnleggende/tokens/z-index',
            ],
        },
        {
            type: 'category',
            label: 'Retningslinjer',
            collapsed: true,
            items: [
                { type: 'doc', id: 'grunnleggende/typografi', label: 'Typografi og fonter' },
                { type: 'doc', id: 'retningslinjer/farger/farger', label: 'Fargebruk' },
                'retningslinjer/farger/fargemodus',
                { type: 'doc', id: 'grunnleggende/native', label: 'Native i hybridapp' },
            ],
        },
        {
            type: 'category',
            label: 'Mønstre',
            collapsed: true,
            items: [
                'monstre-og-maler/layout',
                'monstre-og-maler/spacing',
                'monstre-og-maler/deaktiverte-tilstander',
                'monstre-og-maler/form-validering',
            ],
        },
        {
            type: 'category',
            label: 'Komponenter',
            collapsed: true,
            items: [
                {
                    type: 'category',
                    label: 'Primitiver',
                    collapsed: false,
                    items: [
                        'komponenter/button',
                        {
                            type: 'category',
                            label: 'Icon',
                            link: { type: 'doc', id: 'komponenter/icon' },
                            items: ['komponenter/icon-analyse'],
                        },
                        'komponenter/interactive-icon',
                        'komponenter/progress-bar',
                        'komponenter/spinner',
                        'komponenter/typografi',
                    ],
                },
                {
                    type: 'category',
                    label: 'Layout',
                    collapsed: false,
                    items: [
                        'komponenter/surface',
                        'komponenter/primitives/stack',
                        'komponenter/primitives/grid',
                    ],
                },
                {
                    type: 'category',
                    label: 'Innhold',
                    collapsed: false,
                    items: [
                        'komponenter/card',
                        'komponenter/tag',
                        'komponenter/accordion',
                        'komponenter/read-more',
                    ],
                },
                {
                    type: 'category',
                    label: 'Overlay',
                    collapsed: false,
                    items: ['komponenter/modal', 'komponenter/popover', 'komponenter/dropdown-menu'],
                },
                {
                    type: 'category',
                    label: 'Navigasjon',
                    collapsed: false,
                    items: ['komponenter/tabs', 'komponenter/pagination'],
                },
                {
                    type: 'category',
                    label: 'Varsling',
                    collapsed: false,
                    items: ['komponenter/message'],
                },
                {
                    type: 'category',
                    label: 'Chip',
                    link: { type: 'doc', id: 'komponenter/chip' },
                    items: [
                        'komponenter/chip/button',
                        'komponenter/chip/removable',
                        'komponenter/chip/radio',
                        'komponenter/chip/checkbox',
                    ],
                },
                {
                    type: 'category',
                    label: 'Skjema',
                    collapsed: false,
                    items: [
                        'komponenter/skjema/checkbox',
                        {
                            type: 'category',
                            label: 'CheckboxGroup',
                            link: { type: 'doc', id: 'komponenter/skjema/checkbox-group' },
                            items: ['komponenter/skjema/checkbox-group-designvalg'],
                        },
                        {
                            type: 'category',
                            label: 'Combobox',
                            link: { type: 'doc', id: 'komponenter/skjema/combobox' },
                            items: ['komponenter/skjema/combobox-html'],
                        },
                        'komponenter/skjema/date-field',
                        'komponenter/skjema/label',
                        'komponenter/skjema/phone-number-field',
                        {
                            type: 'category',
                            label: 'RadioGroup',
                            link: { type: 'doc', id: 'komponenter/skjema/radio-group' },
                            items: ['komponenter/skjema/radio-group-designvalg'],
                        },
                        'komponenter/skjema/select',
                        {
                            type: 'category',
                            label: 'TextField',
                            link: { type: 'doc', id: 'komponenter/skjema/textfield' },
                            items: ['komponenter/skjema/textfield-formatering'],
                        },
                        'komponenter/skjema/textarea',
                        'komponenter/skjema/tooltip',
                        'komponenter/skjema/validation-message',
                    ],
                },
            ],
        },
        {
            type: 'category',
            label: 'Utility-klasser',
            collapsed: true,
            items: ['utility-klasser/oversikt', 'utility-klasser/native', 'utility-klasser/hitbox'],
        },
        {
            type: 'category',
            label: 'Om Indeks',
            collapsed: true,
            items: [
                'ordbok',
                { type: 'doc', id: 'adr/retningslinjer', label: 'Teamkonvensjoner' },
                {
                    type: 'category',
                    label: 'Arkitekturbeslutninger (ADR)',
                    link: { type: 'doc', id: 'adr/oversikt' },
                    collapsed: true,
                    items: [
                        'adr/ADR-DS-001-monorepo-og-byggverktoy',
                        'adr/ADR-DS-002-versjonering-og-publisering',
                        'adr/ADR-DS-003-ci-cd',
                        'adr/ADR-DS-004-web-components',
                        'adr/ADR-DS-005-react-bibliotek',
                        'adr/ADR-DS-006-tokens-og-farger',
                        'adr/ADR-DS-007-fargesystem',
                        'adr/ADR-DS-008-spacing-system',
                        'adr/ADR-DS-009-css-only-styling',
                        'adr/ADR-DS-010-komponentutvikling-og-testing',
                        'adr/ADR-DS-011-dokumentasjon',
                    ],
                },
            ],
        },
    ],
};

export default sidebars;
