...
blå der fyll var i bruk (synlig i [Tag](./tag), som har `neutral` som standardverdi). Nøytral-gruppen
henter derfor `--ix-color-status-fill*` fra `neutral`-tieren (grå), mens `--ix-color-status-surface*`
og `--ix-color-status-border` henter fra `main`-tieren — som allerede er nøytral i verdi (hvit flate,
grå kantlinje), og der det ikke finnes `surface-neutral-*`/`border-neutral-*` å bytte til.
