...
**Delvis avklart for `fill`:** `fill-main-default` er brand-blå, så en nøytral flate ble blå der fyll
var i bruk (synlig i [Tag](./tag), som har `neutral` som standardverdi). Nøytral/default-gruppen
henter derfor nå `--ix-color-status-fill*` fra `neutral`-tieren (grå), mens
`--ix-color-status-surface*` og `--ix-color-status-border` fortsatt henter fra `main`-tieren — som
allerede er nøytral i verdi (hvit flate, grå kantlinje), og der det ikke finnes
`surface-neutral-*`/`border-neutral-*` å bytte til.
