...
<div style={{display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '2rem', padding: '3rem'}}>
  <ix-field tooltip="Tekst over feltet" tooltip-placement="top" tooltip-label="Mer informasjon">
    ...
    <div className="ix-text-field"><input type="text" /></div>
    <span data-field="error"></span>
  </ix-field>
...
- **Send alltid inn `tooltipLabel`** — det er den eneste måten info-knappen får et tilgjengelig navn på riktig språk. Mangler den, vises en konsolladvarsel under utvikling.
...
.ix-tooltip {
  ...
  --ix-tooltip-color: var(--ix-color-foreground-inverse-default);
}
```
