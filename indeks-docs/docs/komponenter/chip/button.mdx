...
`error` gir chipen rød kant. `readOnly` gir en nedtonet grå flate uten klikkrespons. Chipen beholder tab-fokus, så innholdet kan fortsatt leses opp. Er begge satt, vinner `error`.
...
```jsx live
<ix-stack horizontal gap="sm">
  <button class="ix-chip" type="button" data-state="error">Feil</button>
  <button class="ix-chip" type="button" data-state="readonly" aria-disabled="true">Skrivebeskyttet</button>
</ix-stack>
```
...
```tsx live
<HStack gap="sm">
  <Chip error>Feil</Chip>
  <Chip readOnly>Skrivebeskyttet</Chip>
</HStack>
```
...
| `error` | `boolean` | | Viser chipen i feiltilstand med rød kant. Slår `readOnly` |
