...
### Hold teksten på noen få ord

Én kort setning, gjerne bare 1–4 ord. Trenger forklaringen mer plass enn det, hører den i
en `description` under feltet.

- Skriv enkelt, og unngå fagbegreper der du kan
- Det viktigste først
- Ingen punktlister
- Bare ren tekst. HTML, lenker og formatering rendres ikke

### La plasseringen stå med mindre den kolliderer

Standarden er `top`, og panelet flipper selv til motsatt side når det ikke er plass. Overstyr bare når du vet at noe konkret er i veien: et felt øverst i en modal, eller en trigger som ligger tett inntil kanten. Plasseringen settes med `tooltipPlacement` (React) eller `tooltip-placement` (HTML):
...
### Regn med at tooltipen lukker seg selv

- Åpner på hover etter 300 ms, og med en gang på fokus og tapp
- Lukker når pekeren forlater triggeren, når fokus flyttes videre, og på `Escape`
- Forsvinner av seg selv, så den trenger ingen lukkeknapp
- Bare én tooltip er synlig om gangen

Ikke legg noe brukeren skal rekke bort til inni panelet. Det lukker i det pekeren forlater
triggeren, så en lenke eller knapp der er ikke til å treffe.
