...
Denne siden forklarer *i detalj* hvordan `<ix-combobox>` fungerer når du skriver ren HTML, Astro eller et annet ikke-React-miljø. Bruker du React, holder det å lese [Combobox](./combobox), der hele strukturen er skjult bak `Combobox`-komponenten. For bruk, tilstander og API, se [hovedsiden](./combobox).
