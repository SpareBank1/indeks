...
For eksempel bør man skrive "Motta nyhetsbrev" i stedet for "Meld meg på nyhetsbrev". Når labelen beskriver selve valget, blir det enklere å forstå hva som skjer når checkboxen hukes velges.
...
CheckboxGroup skal alltid ha en legend, og hver Checkbox skal ha en label. I enkelte tilfeller kan disse skjules visuelt ved hjelp av `hideLegend` eller `hideLabel`, for eksempel i tabeller der konteksten allerede er gitt av kolonneoverskrifter.
...
Dersom indeterminate-tilstand brukes, må denne formidles korrekt til hjelpemidler (f.eks. via `aria-checked="mixed"`). Dette sikrer at også ikke-visuelle brukere forstår tilstanden.
