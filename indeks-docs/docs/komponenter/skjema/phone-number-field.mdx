...
Komponenten er **to uavhengige felt**: landkode og nummer. I React bindes de med hver sin registrering — `countryField` og `numberField` — som spres rett på delfeltene. Begge indre komponenter (Combobox og TextField) er register-kompatible, så `{...register()}` fungerer uten lim-kode:
