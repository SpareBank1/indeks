...
- En enkeltstående checkbox uten gruppe-kontekst (bruk [Checkbox](./checkbox))
...
Legenden er spørsmålet alternativene svarer på: "Hvordan vil du varsles?", ikke "Varsling".
Uten den står brukeren med en liste avkryssinger og må gjette hva de har til felles.

Gjelder det bare ett av/på-valg, som «Jeg godtar vilkårene», bruk en enkeltstående
[Checkbox](./checkbox).

### Vis alternativene under hverandre

Vertikal liste er hovedregelen. Én kolonne kan skannes nedover, tåler zoom og brekker ikke
på smal skjerm, og alternativer med ulik tekstlengde ligger fortsatt på linje til venstre.

### Velg rekkefølgen bevisst

Sorter logisk eller alfabetisk, så leter ikke brukeren. I lange lister kan de mest brukte
valgene ligge øverst, men vær klar over at det du legger først, blir valgt oftere.

### Del opp lange lister

En liste med tjue avkryssinger er tung å holde oversikt over. Del den i flere grupper med
egne legender, eller la brukeren filtrere seg fram til det som er relevant.

### Velg hvor `name` skal stå

En checkbox-gruppe er en visuell gruppering, ikke et gjensidig utelukkende valg. Derfor
finnes det to måter å navngi feltene, og du velger etter hvordan backend vil ha dataene:

- **`name` på gruppen** gir ett felt med flere verdier: `tjenester=nettbank&tjenester=mobilbank`. Bruk denne når valgene er samme spørsmål besvart flere ganger.
- **`name` per valg** gir ett felt per avkryssing: `nettbank=on&mobilbank=on`. Bruk denne når hvert valg er sitt eget felt i skjemaet, som når backend forventer én boolsk verdi per tjeneste.

To ting å vite når du bruker per-valg-`name`:

- Modellene kan ikke kombineres. Setter du `name` på gruppen også, overskriver web-komponenten alle input-navn med gruppens ved mount. Du får en advarsel i dev.
- `value` må fortsatt være unik per valg i kontrollert og ukontrollert modus, fordi gruppen sporer avkryssing på `value`. Kobler du gruppen med `register()`, eier React Hook Form `checked`, og da kan alle valgene ha `value="on"`.
...
Radio er ikke slik: der må alle inputs dele `name` for at nettleseren skal håndheve at bare
ett valg er aktivt. `RadioGroup` har derfor ingen per-valg-`name`.
...
[Designvalg for CheckboxGroup](./checkbox-group-designvalg) går gjennom avveiningene, og
hva som skiller CheckboxGroup fra RadioGroup.
...
- Bruk checkbox kun for uavhengige valg. Er valgene gjensidig utelukkende, bruk [RadioGroup](./radio-group)
...
| `name` | `string` | Nei | | Felles `name` på alle inputs, så valgene sendes inn som flere verdier under ett felt. Overskriver `name` satt per valg . Se [Velg hvor `name` skal stå](#velg-hvor-name-skal-stå) |
...
| `name` | `string` | Nei | Eget `name` på denne checkboxen, så valget blir sitt eget felt i skjemaet. Utelates når gruppen har `name` . Se [Velg hvor `name` skal stå](#velg-hvor-name-skal-stå) |
...
| `label` | `string` | Ja | | Synlig labeltekst |
| `name` | `string` | Nei | | Eget `name` på denne checkboxen. Vinner over gruppens context-navn, men bare når gruppen selv ikke har `name` . Se [Velg hvor `name` skal stå](#velg-hvor-name-skal-stå) |
...
I tillegg støttes alle standard HTML input-attributter (`id`, `autoFocus`, `defaultChecked`, `aria-*`, osv.) som settes direkte på komponenten og sendes videre til `<input>`. `value`, `type`, `size`, `children` og `onChange` håndteres av komponenten.
...
- [Label](./label) — labeltekst for gruppen og for hvert alternativ
