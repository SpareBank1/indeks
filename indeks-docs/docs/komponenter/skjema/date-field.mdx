...
Brukeren ser og taster `dd.mm.åååå`. Selve verdien er ISO (`åååå-mm-dd`). Det er den som sendes ved innsending, og den som ligger i `event.target.value` i `onChange`. Det unngår den amerikanske `mm/dd/åååå`-tvetydigheten samtidig som datoen er maskinlesbar.
:::
...
I feltet taster brukeren datoen. Feltet er skilletegn-bevisst og fleksibelt: du kan taste `1.1.2026` like gjerne som `01.01.2026` (eller `01012026` uten skilletegn). Teksten står urørt mens du skriver. Når feltet mister fokus, normaliserer det til `dd.mm.åååå` og nullutfyller enkeltsifret dag og måned (`1.1.2026` → `01.01.2026`). Året kan tastes med fire sifre (`2026`) eller to sifre som utvides til `20xx`, så `1.1.26` blir `01.01.2026`. Kalenderknappen til høyre åpner enhetens innebygde datovelger. På desktop kaller knappen `showPicker()`; på touch fanges tappet av en gjennomsiktig native date-input som ligger oppå knappen, så kalenderen åpnes også der `showPicker()` ikke er støttet.
...
Som standard fokuserer et tapp i feltet for manuell tasting, og kalenderen åpnes via knappen. Med `nativePickerOnMobile` (HTML: `data-native-picker-mobile`) åpner et tapp _hvor som helst_ i feltet enhetens innebygde datovelger i stedet. Flagget er **kun aktivt på touch-enheter** — på desktop har det ingen effekt, så du kan sette det uten å endre skrivbar-oppførselen der. Test det med mobil-emulering i nettleseren.
...
<ix-stack>
  <ix-field>
    <label for="fodt-mobile">Fødselsdato</label>
    <span data-field="description">dd.mm.åååå</span>
    <ix-date-field class="ix-date-field" data-open-label="Åpne kalender" name="fodt" data-native-picker-mobile>
      <div class="ix-text-field">
        <input id="fodt-mobile" inputmode="numeric" data-format="date" />
      </div>
    </ix-date-field>
    <span data-field="error" aria-live="polite"></span>
  </ix-field>
...
`onChange` er event-basert (ISO i `event.target.value`) og `ref` er en register-kompatibel proxy, så feltet bindes som et hvilket som helst annet — spre `{...register('felt')}` rett på. RHF leser/skriver ISO via proxy-en (`reset`/`setValue`/`defaultValues` fungerer), og `onBlur` gir touched-state.
...
Når gyldige datoer har en naturlig grense (f.eks. ikke frem i tid), sett `min`/`max` slik at OS-kalenderen sperrer dem. Regler som ikke kan uttrykkes som et intervall, som enkeltdager eller helger, valideres på blur eller innsending med en `errorMessage`.
...
| `data-native-picker-mobile` | Nei | Lar et tapp hvor som helst i feltet åpne OS-velgeren. Kun aktivt på touch |
...
| `onChange` | `ChangeEventHandler<HTMLInputElement>` | Nei | | Event-basert. ISO-verdien ligger i `event.target.value` (tom streng når ufullstendig), så `{...register('felt')}` kan spres rett på |
| `onBlur` | `FocusEventHandler<HTMLInputElement>` | Nei | | Kalles når fokus forlater feltet (RHF touched-state, `mode: 'onBlur'`) |
| `min` | `string \| number` | Nei | | Tidligste valgbare dato i ISO |
| `max` | `string \| number` | Nei | | Seneste valgbare dato i ISO |
...
| `ref` | `Ref<HTMLInputElement>` | Nei | | Register-kompatibel proxy: `ref.value` gir/setter ISO, `ref.focus()` fokuserer feltet. (Ikke `ix-date-field`-elementet — verdien eies av web-komponenten) |

I tillegg støttes øvrige HTML-attributter (`aria-*`, `data-*`, `tabIndex`, osv.) som settes direkte på komponenten. De sendes videre til `<ix-date-field>`-**host-elementet**, ikke den native inputen — web-komponenten eier all ARIA/tastatur/state der.
