...
I feltet taster brukeren datoen, som formateres fortløpende til `dd.mm.åååå` — punktumet dukker opp med én gang en gruppe er full (`24` → `24.`, `2412` → `24.12.`), så brukeren aldri trenger å taste skilletegnet selv. Kalenderknappen til høyre åpner enhetens innebygde datovelger. På desktop kaller knappen `showPicker()`; på touch fanges tappet av en gjennomsiktig native date-input som ligger oppå knappen, så kalenderen åpnes robust.
