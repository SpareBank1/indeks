...
Brukeren ser og taster `dd.mm.åååå`. Selve verdien — det som sendes ved innsending og som ligger i `event.target.value` i `onChange` — er ISO (`åååå-mm-dd`). Det unngår den amerikanske `mm/dd/åååå`-tvetydigheten samtidig som datoen er maskinlesbar.
:::
