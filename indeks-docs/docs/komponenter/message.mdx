...
- **Fokusindikator** — meldingen får ingen ring rundt seg selv; kun lukkeknappen har sin egen `:focus-visible`-ring (fra `InteractiveIcon`). Lenker bruker `:focus-visible`.
- **Lukkeknappen følger meldingens status** — lukkeknappen er en `InteractiveIcon` uten egen `status`, og arver derfor meldingens `data-status`. Pekehvil/trykk på krysset vises i meldingens status-farge automatisk.
