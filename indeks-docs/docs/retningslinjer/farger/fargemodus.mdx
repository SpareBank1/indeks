...
Bruk `.ix-scheme-auto` på `<body>`. Da følger siden det brukeren har valgt i operativsystemet, og dere får dark og light mode uten ekstra arbeid:

```html
<body class="ix-body ix-scheme-auto">
    ...
</body>
```
...
- **`.ix-scheme-light`** — tvinger light mode.
- **`.ix-scheme-dark`** — tvinger dark mode.
- **`.ix-scheme-auto`** — auto: følger brukerens systempreferanse via `prefers-color-scheme`. Dark når OS/nettleser står i dark mode, ellers light.

## Tving én modus

Hvis en flate av en god grunn alltid skal være lys eller mørk, kan `.ix-scheme-light` eller `.ix-scheme-dark` brukes i stedet. Vær oppmerksom på at dette kan bryte forventningen til brukeren hvis andre flater følger systempreferansen.

```html
<body class="ix-body ix-scheme-light">
    ...
</body>
```
