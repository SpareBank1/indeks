...
Utility-klassene for Grid i Indeks gjør det enkelt å definere rader, kolonner og gap mellom elementer. For en fullstendig oversikt over tilgjengelige Grid-klasser, se [Grid-dokumentasjonen](../grunnleggende/tokens/utilities#grid).
...
Utility-klassene for Flex i Indeks gir mulighet for fleksibel justering og fordeling av elementer i en rad eller kolonne. For en fullstendig oversikt over tilgjengelige Flex-klasser, se [Flex-dokumentasjonen](../grunnleggende/tokens/utilities#flex).
...
`VStack` og `HStack` er komponenter som forenkler henholdsvis vertikal og horisontal layout. Komponentene bruker Flex under panseret og har `gap: var(--ix-spacing-md)` som standard.
