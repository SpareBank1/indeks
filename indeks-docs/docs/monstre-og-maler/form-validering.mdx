...
:::note `Combobox` med `register()` — initialverdi
Bruker du `register()` (ikke `<Controller>`) og vil vise et forhåndsvalgt alternativ: sett `defaultValue` på `Combobox` i tillegg til `defaultValues` i `useForm`, og hold dem i synk. `defaultValues` alene pushes ikke inn i web-komponenten.
:::
