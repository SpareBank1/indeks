...
Temaet styres med CSS-klasser på rotelementet: `ix-scheme-light` og `ix-scheme-dark` tvinger henholdsvis lyst og mørkt tema. `ix-scheme-auto` følger brukerens OS-preferanse via `prefers-color-scheme`. Uten klasse er light mode default. Klassene deler `ix-scheme-`-prefiks med varierende suffiks, og mapper mot CSS-egenskapen `color-scheme` / `prefers-color-scheme`.
