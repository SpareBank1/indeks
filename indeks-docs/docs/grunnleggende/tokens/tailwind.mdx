...
Vi anbefaler ikke Tailwind i nye prosjekter. Indeks har egne utility-klasser med
`ix-`-prefiks som dekker det samme, og de krever ingen byggesteg hos deg. Men vi vet at
flere team allerede har Tailwind, og da skal du ikke måtte skrive
`bg-[var(--ix-color-fill-main-default)]` for å bruke designsystemet.

Derfor genererer vi en Tailwind 4-temafil sammen med resten av pakkene. Den kobler
Tailwinds navnerom til Indeks-variablene, slik at `bg-fill-main-default` og `p-md` gir
riktige verdier.
...
Komponent-variablene (`--ix-color-component-button-*`) er holdt utenfor. De finnes for at
våre egne komponenter skal kunne temes, og vi vil ikke at koden din skal binde seg til
dem. Trenger du én likevel, se `var()`-formen under.
