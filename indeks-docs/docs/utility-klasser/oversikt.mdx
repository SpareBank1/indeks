...
Spacing i Indeks bruker faste px-verdier som justeres på tre breakpoints (mobil, tablet fra 768px, desktop fra 1024px). De minste verdiene (`2xs`–`lg`) er like på alle breakpoints; kun `xl`–`5xl` øker på større skjermer. Verdiene kan overstyres av `density`-modifierne. Se [spacing-skalaen](/docs/grunnleggende/tokens/spacing) for alle verdier.
...
Outline brukes ved fokus på skjemaelementer. Outline defineres direkte på elementet og er satt til `transparent` når elementet ikke er i fokus for å unngå hopping i layouten. På fokus endres `outline-color` til `var(--ix-outline-default-visible)`.

| CSS-klasse            | Verdi                                                                                    |
| --------------------- | ---------------------------------------------------------------------------------------- |
| `.ix-outline-default` | `outline: var(--ix-outline-width-default) solid transparent;`<br/> `outline-offset: var(--ix-outline-offset-default);`   |
| `.ix-outline-default:focus-visible` | `outline-color: var(--ix-outline-default-visible);`   |
...
Bakgrunn på en side settes med `background`, flater som ligger oppå den med `surface`, og mindre elementer med `fill`. Se [Farger](../retningslinjer/farger) for når du bruker hvilken.
