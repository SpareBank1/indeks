...
describe('IxIcon', () => {
    describe('--ii-icon-url', () => {
        it('setter --ii-icon-url naar name er satt', () => {
            const icon = createIcon(`<ix-icon name="home"></ix-icon>`);
            expect(icon.style.getPropertyValue('--ii-icon-url')).toContain('home.svg');
        });

        it('bruker name-attributtet direkte som Material Design-filnavn', () => {
            const icon = createIcon(`<ix-icon name="menu"></ix-icon>`);
            expect(icon.style.getPropertyValue('--ii-icon-url')).toContain('menu.svg');
        });

        it('fjerner --ii-icon-url naar name ikke er satt', () => {
    ...
    describe('data-size', () => {
        it('har data-size="sm" naar data-size="sm" er satt direkte', () => {
            const icon = createIcon(`<ix-icon name="home" data-size="sm"></ix-icon>`);
    ...
    describe('tilgjengelighet', () => {
        ...
        it('setter role="img" og fjerner aria-hidden naar aria-label er satt', () => {
            const icon = createIcon(`<ix-icon name="home" aria-label="Gå til forsiden"></ix-icon>`);
        ...
        it('setter role="img" og fjerner aria-hidden naar aria-labelledby er satt', () => {
            const icon = createIcon(`<ix-icon name="home" aria-labelledby="tittel-id"></ix-icon>`);
            expect(icon.getAttribute('role')).toBe('img');
            expect(icon.hasAttribute('aria-hidden')).toBe(false);
        });
