...
const preview: Preview = {
    ...
    decorators: [
        (Story, context) => {
            const { scheme, accent, device } = context.globals;

            const classname = `${device === 'mobile' ? 'indeks-mobile' : 'indeks-desktop'}`;

            return (
                <div id="stories-container">
                    {(scheme === 'both' || scheme === 'light') && (accent === 'both' || accent === 'default') && (
                        <div className={`storybook-docs-content-container ${classname}`}>
                            <h3>Light mode</h3>
                            <Story />
                        </div>
                    )}
                    {(scheme === 'both' || scheme === 'light') && (accent === 'both' || accent === 'accent') && (
                        <div className={`storybook-docs-content-container ${classname}`}>
                            <h3>Light mode - Accent</h3>
                            <Story />
                        </div>
                    )}
                    {(scheme === 'both' || scheme === 'dark') && (accent === 'both' || accent === 'default') && (
                        <div className={`storybook-docs-content-container ix-dark-mode ${classname}`}>
                            <h3>Dark mode</h3>
                            <Story />
                        </div>
                    )}
                    {(scheme === 'both' || scheme === 'dark') && (accent === 'both' || accent === 'accent') && (
                        <div className={`storybook-docs-content-container ix-dark-mode ${classname}`}>
