...
describe('InteractiveIcon', () => {
    it('rendrer som button med klassen ix-interactive-icon', () => {
        render(<InteractiveIcon name="hjem" aria-label="Hjem" />);
    ...
    it('setter aria-label på knappen', () => {
        render(<InteractiveIcon name="hjem" aria-label="Gå hjem" />);
    ...
    it('rendrer ix-icon med riktig name', () => {
        render(<InteractiveIcon name="lukk" aria-label="Lukk" />);
        const icon = screen.getByRole('button').getElementsByClassName('ix-icon')[0];
        expect(icon).toBeDefined();
        expect(icon.getAttribute('name')).toBe('lukk');
    });

    it('setter data-status for info', () => {
        render(<InteractiveIcon name="hjem" aria-label="Hjem" status="info" />);
        expect(screen.getByRole('button').getAttribute('data-status')).toBe('info');
    });

    it('setter data-status for danger', () => {
        render(<InteractiveIcon name="hjem" aria-label="Hjem" status="danger" />);
    ...
    it('videresender onClick', () => {
        const onClick = vi.fn();
        render(<InteractiveIcon name="hjem" aria-label="Hjem" onClick={onClick} />);
        fireEvent.click(screen.getByRole('button'));
        expect(onClick).toHaveBeenCalledOnce();
    });

    it('videresender disabled', () => {
        render(<InteractiveIcon name="hjem" aria-label="Hjem" disabled />);
